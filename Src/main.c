...
// Variables para UART
uint8_t rx_data; // Variable para recibir 1 byte por UART
char tx_buffer[50]; // Buffer para recibir y luego transmitir mensajes por UART
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART2)
  {
    ...
    // Si el usuario envía '@', responder con 'X'
    if (rx_data == '@')
    {
      reply = 'X';
    }
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 1500) // Cada 500 ms
    {
    ...
    if (button_flag == 1)
    {
      ...
      led_ext_off_time = HAL_GetTick() + 6000;

      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Presionaste el boton azul.\r\n");
